# -*- coding: utf-8 -*-
"""
Created on Jan  1 15:42:29 2022
...
def nested_cross_validate_model(feature_df,clinical_features,y_event,y_time, n_folds=5):
    ...
    for train_test_ix, validation_idx  in k_outer_fold.split(feature_df):
        ...
        tuner.search(x = [train_rna, train_clic, train_event, train_time], 
        ...
        history = best_model.fit(x = [train_test_rna, train_test_clic, train_test_event, train_test_time], 
                     batch_size=batch_size, 
                     epochs=epochs,
...
c_index, best_hyperparameters, pred_risk_list, shap_values, time, event, tuner_summary = nested_cross_validate_model(pd.DataFrame(np.array(feature_dataset)),pd.DataFrame(np.array(clinical_features)),pd.DataFrame(np.array(y_event)),pd.DataFrame(np.array(y_survival_time)), n_folds=5)
...
with open('C:/Users/suraj/OneDrive/Desktop/tuned-HVAEResults/X-VAE-gene-clinical-08-final-result/H-VAE-Coxsaved_Results.pkl','wb') as res:
  pickle.dump((
