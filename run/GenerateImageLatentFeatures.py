...
def nested_cross_validate_model(radio_genomics_df,y_event,y_time, n_folds=5):
    val_c_index, latent_features, pred_risk_list, time, event = list(),list(), list(), list(), list()
    ...
    for train_test_ix, validation_idx  in k_outer_fold.split(radio_genomics_df):
        ...
        tuner.search(x = [train_rad, train_event, train_time],
        ...
        history = best_model.fit(x = [train_test_rad, train_test_event, train_test_time], 
                     batch_size=batch_size, 
                     epochs=epochs,
        ...
        i = i+1
    return val_c_index, latent_features, pred_risk_list, time, event


val_c_index, latent_features, pred_risk_list, time, event = nested_cross_validate_model(feature_dataset,y_event,y_survival_time, n_folds=5)
