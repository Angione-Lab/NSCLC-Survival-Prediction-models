# -*- coding: utf-8 -*-
"""
Created on Mon Jan  1 15:06:57 2022
...
def nested_cross_validate_model(radio_genomics_df,clinical_features,y_event,y_time):
    c_ipw, c_index, best_hyperparameters, shap_values, pred_risk_list,high_risk_features_list, high_risk_clinical_list, time, event = list(), list(),list(), list(), list(), list(), list(), list(), list()
    ...
    for train_test_ix, validation_idx  in k_outer_fold.split(radio_genomics_df, y_event):
        ...
        tuner.search(
            x=[train_rad,train_clic,train_event,train_time],
        ...
        history = c_model.fit(
            x=[train_test_rad,train_test_clic,train_test_event,train_test_time],
        ...
        et_train = np.array([(np.array(train_test_event)[i], np.array(train_test_time)[i]) for i in range(len(train_test_event))],
                         dtype = [('e', bool), ('t', float)])
        
        et_test = np.array([(np.array(validation_test_event)[i], np.array(validation_time)[i]) for i in range(len(validation_test_event))],
                         dtype = [('e', bool), ('t', float)])
        
        c_ipw.append(concordance_index_ipcw(et_train, et_test, pred_risk.reshape(-1))[0])
...
c_ipw, c_index, best_hyperparameters, pred_risk_list, shap_values, time, event, high_risk_features_list, high_risk_clinical_list = nested_cross_validate_model(pd.DataFrame(np.array(feature_dataset)),clinical_features,y_event,y_survival_time)
...
for i in range(5):
    gene_sha = gene_sha.append(pd.DataFrame(shap_obj[i][0][0][0]))
...
for i in range(5):
    img_gene_sha = img_gene_sha.append(pd.DataFrame(shap_obj[i][0][0][0]))
 
#clinical shap values
clinical_shap = pd.DataFrame()
for i in range(5):
    clinical_shap = clinical_shap.append(pd.DataFrame(shap_obj[i][0][0][1]))


img_rna_obj = pd.DataFrame()
for i in range(5):
    img_rna_obj = img_rna_obj.append(pd.DataFrame(high_risk_features_list[i]))


clinical_obj = pd.DataFrame()
for i in range(5):
    clinical_obj = clinical_obj.append(pd.DataFrame(high_risk_clinical_list[i]))    
